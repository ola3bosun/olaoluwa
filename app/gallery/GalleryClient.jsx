...
export default function GalleryClient({ initialProjects }) {
  ...
  const allProjects =
    initialProjects?.length > 0 ? initialProjects : fallbackProjects;
  ...
  const handleCategorySwitch = (targetCat) => {
    ...
    const images = gsap.utils.toArray(".gallery-image", containerRef.current);
    const texts = gsap.utils.toArray(".gallery-text", containerRef.current);
  ...
  useGSAP(
    () => {
      ...
      gsap.set(images, { opacity: 0, zIndex: 0, clearProps: "transform" });
      gsap.set(texts, {
        opacity: 0,
        zIndex: 0,
  ...
  const gotoSlide = (targetIndex, direction) => {
    ...
    const currentImg = images[currentIndexRef.current];
    const nextImg = images[targetIndex];
    const currentText = texts[currentIndexRef.current];
  ...
  useGSAP(
    () => {
      ...
      const handlePrev = () => {
        if (filteredProjects.length === 0) return;
        const prevIdx =
          (currentIndexRef.current - 1 + filteredProjects.length) %
    ...
    >
      ...
      <div className="absolute inset-0 w-full h-full z-0 pointer-events-none">
        ...
        {filteredProjects.map((project, idx) => (
          <div
            key={`img-${project._id}`}
            className="absolute inset-0 w-full h-full gallery-image"
          >
            <Image
      ...
      <nav className="absolute top-0 left-0 w-full p-4 md:px-8 md:pt-6 flex justify-between items-center uppercase font-mono text-xs md:text-sm tracking-widest z-50 pointer-events-none">
        <div className="w-32 flex justify-start pointer-events-auto">
          <TransitionLink
            href="/"
            className="hover:opacity-50 transition-opacity cursor-pointer drop-shadow-md"
          >
            &lt;- BACK
          </TransitionLink>
        </div>
          ...
          return (
            <button
              ...
              className={`relative w-full aspect-[4/3] rounded-md overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] cursor-pointer pointer-events-auto ${
                isActive
                  ? "opacity-100 scale-110 shadow-2xl shadow-black/50 grayscale-0"
                  : "opacity-30 scale-95 hover:opacity-70 grayscale"
              }`}
            >
              <Image
                src={project.imageUrl}
      ...
      <div className="absolute bottom-0 right-0 p-4 md:p-8 flex flex-col items-end w-full h-[30vh] overflow-hidden z-20 pointer-events-none">
        {filteredProjects.map((project, idx) => (
          <div
            ...
            className="absolute bottom-8 right-8 flex flex-col items-end text-right font-mono text-xs md:text-sm uppercase tracking-widest text-white drop-shadow-md gallery-text"
          >
            <span className="font-bold text-xl md:text-3xl mb-1">
              {project.title}
            </span>
            <span className="opacity-80">{project.location}</span>
            <span className="opacity-60 mt-2 text-[10px] md:text-xs">
              [{project.year}]
            </span>
          </div>
        ))}
