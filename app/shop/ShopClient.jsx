...
// This ensures the page doesn't crash if your Sanity database is empty during testing
...
export default function ShopClient({ initialInventory }) {
  ...
  // Use Sanity data if it exists, otherwise fall back to the dummy data
  const inventory = initialInventory?.length > 0 ? initialInventory : fallbackInventory
  ...
  // Calculate total: uses Sanity's numericPrice if available, otherwise cleans the fallback string
  const cartTotal = cart.reduce((total, item) => {
    const itemPrice = item.numericPrice ? item.numericPrice : parseInt(item.price.replace(/\D/g,''))
